    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
                ...
                <a href="{{ route('admin.gaji-report.print', array_filter(['bulan' => request('bulan')])) }}"
                    class="btn btn-outline-secondary" target="_blank">Cetak PDF</a>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($gajis as $gaji)
                            <tr>
                                ...
                                <td>Rp  {{ number_format($gaji->detail['honor'] ?? $gaji->total_honor, 0, ',', '.') }}</td>
                                <td>Rp {{ number_format($gaji->detail['transport'] ?? 0, 0, ',', '.') }}</td>
                                <td>Rp {{ number_format($gaji->detail['bpjs'] ?? 0, 0, ',', '.') }}</td>
                                <td>Rp {{ number_format($gaji->detail['potongan_lain'] ?? 0, 0, ',', '.') }}</td>
                                <td>Rp {{ number_format($gaji->total_gaji, 0, ',', '.') }}</td>
                                <td>
                                    <button class="btn btn-sm btn-secondary btn-detail" data-id="{{ $gaji->id }}">Rincian</button>
                                </td>
                                <td>
                                    <form action="{{ route('admin.gaji-report.destroy', $gaji) }}" method="POST"
                            ...
                            <tr class="gaji-detail-row d-none" data-id="{{ $gaji->id }}">
                                <td colspan="10">
                                    ...
                                    <div><strong>Honor per Jam:</strong> Rp {{ number_format($gaji->honor_per_jam ?? 0, 0, ',', '.') }}
                                    </div>
                                    <div><strong>Kehadiran:</strong> {{ $gaji->kehadiran }}</div>
                                    <div><strong>Rincian Honor:</strong> Rp
                                        {{ number_format($gaji->detail['honor'] ?? $gaji->total_honor, 0, ',', '.') }}</div>
                                    <div><strong>Transport:</strong> Rp
                                        {{ number_format($gaji->detail['transport'] ?? 0, 0, ',', '.') }}</div>
                                    <div><strong>BPJS:</strong> Rp {{ number_format($gaji->detail['bpjs'] ?? 0, 0, ',', '.') }}</div>
                                    <div><strong>Potongan Lain:</strong> Rp
                                        {{ number_format($gaji->detail['potongan_lain'] ?? 0, 0, ',', '.') }}</div>
